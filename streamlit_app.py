...
def render_help() -> None:
    st.caption("이 시스템을 실제 업무에 어떻게 쓰는지 안내합니다. 처음이라면 ‘3분 퀵스타트’부터 보세요.")

    t1, t2, t3, t4, t5 = st.tabs(
        ["🚀 퀵스타트", "👥 역할별 사용법", "🗂 시나리오", "📊 차트 읽는 법", "🔌 내 데이터 · FAQ"])

    # ── 1) 퀵스타트 ────────────────────────────────────────────
    with t1:
        st.markdown("#### 3분 퀵스타트")
        steps = [
            ("1", "📥 과거 데이터", "샘플 데이터로 시작하세요. 상단 KPI와 **병목 후보** 막대에서 "
                                 "체류시간이 가장 긴 공정을 확인합니다."),
            ("2", "🔎 원인 분석", "그 병목이 **왜** 생겼는지 봅니다. 가동률 100%에 가까운 설비가 "
                                "진짜 병목이고, 상류의 ‘블로킹’이 그 증거입니다."),
            ("3", "📈 개선 what-if", "주조 **순서만** 바꿨을 때 얼마나 좋아지는지 비교합니다. "
                                    "윈도우 크기를 움직여 보세요."),
            ("4", "🧭 라우팅 가이던스", "신규 슬래브 조건을 넣으면 **이동 방향·조치**와 그 근거를 제시합니다."),
        ]
        for n, title, desc in steps:
            c = st.columns([0.06, 0.94])
            c[0].markdown(f"### {n}")
            c[1].markdown(f"**{title}** — {desc}")
        st.info("왼쪽 메뉴는 **분석 순서(A→E)** 대로 배치되어 있습니다. 위에서 아래로 따라가면 "
                "‘무슨 일이 있었나 → 왜 → 어떻게 개선 → 지금 뭘 할까’ 흐름이 됩니다.")

        st.markdown("#### 이 시스템으로 답할 수 있는 질문")
        st.markdown(
            "- 우리 공정에서 **병목은 어디이고, 시간대별로 옮겨 다니는가?**\n"
            "- 그 병목은 **설비 탓인가, 전환(셋업) 탓인가, 상류 변동 탓인가?**\n"
            "- **주조 순서를 바꾸면** 리드타임·처리량이 얼마나 좋아지는가?\n"
            "- 지금 들어온 이 슬래브는 **어디로 보내고, 언제 주조**해야 하는가?")

    # ── 2) 역할별 ─────────────────────────────────────────────
    with t2:
        st.markdown("#### 역할별 추천 사용법")
        roles = [
            ("🏭 조업 · 오퍼레이터", ["🧭 라우팅 가이던스", "🔎 원인 분석"],
             "지금 들어온 슬래브를 어디로 보낼지 판단할 때 가이던스를 참고하고, "
             "추천 **근거**를 확인해 현장 감각과 비교하세요. 추천은 참고용이며 최종 결정은 담당자가 합니다."),
            ("📋 생산계획 · 스케줄", ["📈 개선 what-if", "🧩 최적화 예시"],
             "캐스트 편성(주조 순서)을 검토할 때 전환 횟수와 리드타임의 **트레이드오프**를 확인하세요. "
             "‘전체 그룹핑’처럼 한 목적만 밀면 부작용이 생기는 것도 함께 보입니다."),
            ("📈 공정분석 · 기술", ["📥 과거 데이터", "🔎 원인 분석"],
             "기간·강종으로 필터해 병목과 원인을 정량 분석하고, 사유코드·전환 영향으로 개선 과제를 도출하세요."),
            ("💡 관리자 · 의사결정", ["📥 과거 데이터", "📈 개선 what-if"],
             "상단 KPI(리드타임·처리량·병목)와 개선 시나리오의 기대효과를 중심으로 보세요."),
        ]
        for name, pages, desc in roles:
            with st.container(border=True):
                st.markdown(f"**{name}**")
                st.caption("추천 페이지: " + "  ·  ".join(pages))
                st.write(desc)

    # ── 3) 시나리오 ───────────────────────────────────────────
    with t3:
        st.markdown("#### 자주 쓰는 업무 시나리오")
        with st.expander("① 지난주 물류에서 병목을 찾고 원인을 보고해야 한다", expanded=True):
            st.markdown(
                "1. **📥 과거 데이터** → 기간을 지난주로 설정, 필요하면 강종 필터\n"
                "2. **병목 후보** 막대에서 체류시간 1위 공정 확인 → **설비 가동률 추이**에서 포화 구간 확인\n"
                "3. **🔎 원인 분석** → 체류 분해로 그 시간이 *대기/처리/블로킹* 중 무엇인지 판별\n"
                "4. **사유코드**·**강종 전환 영향**에서 지연 기여도 상위 항목을 보고 근거로 사용\n"
                "5. **히트맵**으로 병목이 특정 시간대에 집중되는지 확인")
        with st.expander("② 캐스트 편성(주조 순서)을 개선하고 싶다"):
            st.markdown(
                "1. **🔎 원인 분석**에서 전환 셋업이 병목에 주는 영향 확인\n"
                "2. **📈 개선 what-if** → 윈도우 크기를 조절하며 전환↓과 리드타임 개선이 **함께** 되는 지점 탐색\n"
                "3. ‘전체 그룹핑’ 결과와 비교해 **과도한 묶기의 부작용**(상류 버스트) 확인\n"
                "4. **🧩 최적화 예시**에서 폭 전이(coffin)·납기 제약을 켜고 끄며 현실적 순서를 검토")
        with st.expander("③ 지금 들어온 슬래브를 어디로 보낼지 판단한다"):
            st.markdown(
                "1. **🧭 라우팅 가이던스** → 신규 슬래브(강종·폭·납기)와 현재 상태(몰드 강종·대기열·가동률) 입력\n"
                "2. 추천 **조치**와 **근거**, 예상 셋업·대기 확인\n"
                "3. 납기 임박 여부를 바꿔 보며 판단이 어떻게 달라지는지 비교\n"
                "4. 현장 판단과 다르면 그 이유를 기록해 주세요 — 규칙 보정에 사용됩니다")
        with st.expander("④ 설비 증설·버퍼 확대를 검토한다"):
            st.markdown(
                "1. **🔎 원인 분석**의 **설비 상태 분해**에서 각 설비의 가동/블로킹/유휴 비율 확인\n"
                "2. 유휴(starving)가 큰 설비는 증설해도 효과가 작습니다 — **포화된 병목부터**\n"
                "3. **🔄 시뮬레이션**에서 파라미터를 바꿔 흐름 변화를 실험")

    # ── 4) 차트 읽는 법 ───────────────────────────────────────
    with t4:
        st.markdown("#### 해석 가이드")
        st.markdown("**체류시간 분해 (대기 / 처리 / 블로킹)**")
        st.markdown(
            "- **대기(큐)** 가 길다 → 앞에 일감이 쌓임. 그 설비 자체의 용량 부족 가능성\n"
            "- **처리**가 길다 → 작업시간·셋업 문제 (전환 셋업 확인)\n"
            "- **블로킹**이 길다 → 그 공정 잘못이 아니라 **하류가 막힌 것**. 하류를 봐야 함")
        st.divider()
        st.markdown("**가동률 · 설비 상태 분해**")
        st.markdown(
            "- 가동률이 **90~100%** 인 설비 = 사실상 병목. 전체 처리량을 제약\n"
            "- **유휴(starving)** 가 큰 설비 = 병목에 막혀 노는 중 → 여기 증설해도 효과 미미\n"
            "- **블로킹** 비중이 큰 설비 = 하류(병목) 대기 중 → 병목 해소가 곧 이 설비 개선")
        st.divider()
        st.markdown("**히트맵 (시간대별 병목 이동)**")
        st.markdown(
            "- 가로=시간대, 세로=공정. 진할수록 체류시간 김\n"
            "- 한 행이 계속 진하면 **상시 병목**, 특정 열만 진하면 **일시적 부하**(투입 집중·정비 등)\n"
            "- 빈칸은 그 시간대에 해당 공정 통과 이력이 없다는 뜻")
        st.divider()
        st.markdown("**개선 what-if 비교**")
        st.markdown(
            "- 기준(도착순)=100%로 놓고, 낮을수록 개선입니다\n"
            "- 전환·셋업만 좋아지고 리드타임이 나빠지면 **과최적화** 신호입니다")

    # ── 5) 내 데이터 · FAQ ────────────────────────────────────
    with t5:
        st.markdown("#### 내 데이터로 사용하기")
        st.markdown(
            "**📥 과거 데이터** 페이지에서 `CSV 업로드`를 선택하면 실제 로그로 모든 분석이 동작합니다. "
            "1행 = *heat 1개가 공정 1곳을 통과한 기록* 형식입니다.")
        st.markdown("**필수 컬럼**")
        st.code("heat_no, grade, stage, enter_time, exit_time", language=None)
        st.markdown("**있으면 분석이 깊어지는 컬럼**")
        st.code("start_time, proc_end_time  (→ 대기/처리/블로킹 분해)\n"
                "reason_code               (→ 지연 원인 분석)\n"
                "width_mm, thickness_mm, due_date, equipment", language=None)
        st.caption("상세 형식·전달 방법은 저장소의 `docs/DATA_REQUEST.md` 문서를 현업/IT에 전달하세요.")

        st.divider()
        st.markdown("#### ⚠️ 신뢰도 — 지금은 어디까지 믿어도 되나")
        st.warning(
            "**현재 화면의 수치는 샘플(가정) 데이터 기준입니다.**\n\n"
            "- 공정 처리시간·용량, 셋업 시간, 최적화 가중치는 **아직 가정값**입니다\n"
            "- 실제 로그를 업로드하면 **가시화·병목·원인 분석은 실제값**으로 동작합니다\n"
            "- **개선 효과(%)와 라우팅 추천**은 현업 인터뷰로 제약이 확정된 뒤 신뢰할 수 있습니다\n"
            "- 라우팅 추천은 **참고용**이며, 최종 결정은 담당자가 합니다 (human-in-the-loop)")

        st.divider()
        st.markdown("#### FAQ · 문제 해결")
        with st.expander("업로드했더니 ‘필수 컬럼 누락’ 오류가 납니다"):
            st.write("컬럼명이 정확히 `heat_no, grade, stage, enter_time, exit_time` 인지 확인하세요. "
                     "공정명은 코드(`mold`) 또는 한글 라벨(`몰드`) 모두 인식합니다.")
        with st.expander("‘대기/블로킹 분해 불가’ 라고 나옵니다"):
            st.write("`start_time`·`proc_end_time` 이 없기 때문입니다. 체류시간(총합) 분석은 그대로 되지만, "
                     "대기와 블로킹을 나누려면 처리 시작·완료 시각이 필요합니다.")
        with st.expander("우리 공정 단계가 화면의 8단계와 다릅니다"):
            st.write("현재는 전로→정련(LF/RH)→턴디시→몰드→냉각→야드 기준입니다. 실제 공정 코드 목록을 주시면 "
                     "매핑하거나 단계를 추가할 수 있습니다 (`src/model.py`의 STAGES).")
        with st.expander("숫자가 실제 조업과 다릅니다"):
            st.write("샘플 데이터이거나, 모델 파라미터(처리시간·용량)가 아직 실제와 맞지 않아서입니다. "
                     "실데이터를 주시면 파라미터를 보정(캘리브레이션)합니다.")

        st.divider()
        st.markdown("#### 다음에 추가될 기능")
        st.markdown(
            "- **제약 기반 최적화(CP-SAT)** — 현업 인터뷰로 제약·목적 확정 후, 다수 슬래브를 동시에 배치·시퀀싱\n"
            "- **모델 캘리브레이션** — 실데이터로 처리시간·용량 보정\n"
            "- **실시간 연동** — MES 연계 및 온라인 재계획 (장기)")
        st.caption("피드백·요청은 저장소 이슈 또는 담당자에게 전달해 주세요. "
                   "특히 ‘추천이 현장 판단과 달랐던 사례’가 가장 큰 도움이 됩니다.")
...
PAGES = [
    ...
    ("❓ 도움말", "사용 안내", "도움말 · 이 시스템 활용법", render_help,
     "퀵스타트·역할별 사용법·업무 시나리오·차트 해석·내 데이터 사용법과 FAQ를 담았습니다. "
     "현재 수치의 신뢰 범위(가정 vs 실제)도 여기서 확인하세요."),
]
...
TIPS = {
    ...
    "❓ 도움말": "처음이면 ‘퀵스타트’, 실무 적용은 ‘시나리오’, 숫자 해석은 ‘차트 읽는 법’ 탭을 보세요.",
}
